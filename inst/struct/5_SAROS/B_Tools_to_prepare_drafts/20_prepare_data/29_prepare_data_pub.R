################################################################################
############ Data preparation steps. Try to leave it as generic  ###############
############ as possible to ease reusability across projects     ###############
...
### If you are in need of other formats: install.packages("rio"); library(rio)
### Example analysis using real NIFU project data.
...
dat_survey_raw <-
  ...
  mutate(skole = case_when(skole == "Hegra skole" ~ "Hegra barneskole",
         ...
         resp_status = case_when(statoverall_1==1 ~ "1-Not invited",
                                 ...
                                 statoverall_5==1 ~ "5-Excused from participation"))
