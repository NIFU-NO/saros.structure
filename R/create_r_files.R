...
create_r_files <- function(r_files_out_path,
  ...
  data <- read.csv(file = r_files_source_path)
  if(!all(colnames(data) %in% c("folder_name", "file_name"))) cli::cli_abort("Cannot find columns {.var {c(folder_name, file_name)}} in {.file {r_files_source_path}}.")
  data <- data[!is.na(data$folder_name) &
                 !is.na(data$r_files_source_path), ]
  if(isFALSE(optionals)) data <- data[as.integer(data$optional) == 1L, ]
  ...
  for(i in seq_len(nrow(data))) {
    ...
    if(!file.exists(r_filepath)) cat(prefix_file_scope, data[i, "file_scope"], "\n", file = r_filepath)
  }
